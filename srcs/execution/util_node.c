...
/*   Updated: 2024/10/10 21:30:19 by hlee-sun         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	heredoc_check(t_ASTNode	**node)
{
	 if (node == NULL || *node == NULL)
        return log_errors("heredoc_check: AST node is NULL", "");
...
// 	{
// 		log_errors("Failed to dup2 in action_child", strerror(errno));
// 		close(oldfd);
// 		return (FAIL);
// 	}
// 	close(oldfd);
...
int	dup_and_close(int oldfd, int newfd)
{
    if (oldfd == -1)
    {
        log_errors("dup_and_close: Invalid file descriptor", strerror(errno));
        return (FAIL);
    }

    if (dup2(oldfd, newfd) == -1)
    {
        log_errors("dup_and_close: Failed to duplicate file descriptor", strerror(errno));
        return (FAIL);
    }

    if (close(oldfd) == -1)
    {
        log_errors("dup_and_close: Failed to close old file descriptor", strerror(errno));
        return (FAIL);
    }
